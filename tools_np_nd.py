...
def create_dataset(excel_index):
    # 读取 excel 数据
    df = pd.read_excel(f'data/test{excel_index}_nd.xlsx') # 假设你的数据文件名为 data.xlsx
    # 前 5 列数据读取为 dataX
    dataX = df.iloc[:, :4].values
    # 第 6 列数据读取为 dataY
    dataY = df.iloc[:, 4].values
...
class DataPrepare(Dataset):
    ...
    def __init__(self, train):
        ...
        x_set = train[:, 0:-1]
        x_set = x_set.reshape(x_set.shape[0], 1, 4)
        # 数据类型转为 torch 变量
